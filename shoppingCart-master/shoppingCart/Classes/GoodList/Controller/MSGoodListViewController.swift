...
class MSGoodListViewController: UIViewController {
    
    // MARK: - 属性
    /// 商品模型数组，初始化
    private var goodArray = [MSGoodModel]()
    
    /// 商品列表cell的重用标识符
    private let goodListCellIdentifier = "goodListCell"
    
    /// 已经添加进购物车的商品模型数组，初始化
    private var addGoodArray = [MSGoodModel]()
    ...
    override func viewDidLoad() {
        ...
        for i in 0..<10 {
            ...
            // 字典转模型并将模型添加到模型数组中
            goodArray.append(MSGoodModel(dict: dict))
        }
    ...
    private func prepareUI() {
        ...
        // 注册cell
        tableView.registerClass(MSGoodListCell.self, forCellReuseIdentifier: goodListCellIdentifier)
    }
...
extension MSGoodListViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCellWithIdentifier(goodListCellIdentifier, forIndexPath: indexPath) as! MSGoodListCell
...
// view上的一些事件处理在这个类扩展里
extension MSGoodListViewController {
    ...
    @objc private func didTappedCarButton(button: UIButton) {
        ...
        let shoppingCartVc = MSShoppingCartViewController()
...
// MARK: - MSGoodListCellDelegate代理方法
extension MSGoodListViewController: MSGoodListCellDelegate {
